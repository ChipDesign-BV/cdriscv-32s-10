...
//   +-- QSPI boot loader (POST-SIGNOFF addition, elaborated only when
//   |   BootEnable=1: fills the TCMs from external NOR flash at cold
//   |   reset; a 2:1 mux makes it the temporary data-bus master until
//   |   boot_done, and the core's fetch enable is gated by boot_done.
//   |   With the default BootEnable=0 the loader, the mux and the gate
//   |   fold away and this module is functionally identical to the
//   |   signed-off 2026-08-24 configuration -- see cdriscv_qspi_boot)
...
#(
    ...
    // QSPI boot loader (POST-SIGNOFF addition, see cdriscv_qspi_boot).
    // BootEnable defaults to 0 -- the OPPOSITE of cdriscv-32s-20 --
    // because this design is signed off: with 0 the loader and its mux
    // fold away entirely, the QSPI outputs tie to constants, and the
    // subsystem logic is identical to the signed-off configuration
    // (every existing bench preloads the TCMs and is unaffected).  A
    // chip build that wants flash boot sets BootEnable=1 and must
    // re-verify and re-harden.
    parameter bit          BootEnable  = 1'b0,
    ...
    // QSPI boot flash master (appended, POST-SIGNOFF).  Constant /
    // ignored when BootEnable=0.  qspi_io_* map onto four bidirectional
    // pads at chip level: pad b drives qspi_io_o[b] while
    // qspi_io_oe_o[b] is set and returns on qspi_io_i[b].
  ...
  cdriscv_rst_sync #(.Stages(3)) u_rst_sync (
      .clk_i  (clk_i),
      .rst_ni (rst_ni),
      .rst_no (rst_n_sync)
  );
  ...
  // The bus-facing data_* wires above are what the interconnect sees.
  ...
  // access -- before the loader has verified the image.  BootEnable=0
  // (the default) ties boot_done to 1 and this line reduces to the
  // signed-off original.
  ...
  // ------------------------------------------------------------------
  // QSPI boot loader and the boot-time data-master mux (POST-SIGNOFF)
  // ------------------------------------------------------------------
  ...
  // changes, and the fetch path is untouched.  Until boot_done the
  // loader owns the port; the core sees gnt/rvalid low (it cannot
  // request anyway, since its fetch enable is gated by boot_done
  // above).  The loader's writes therefore travel through the same
  // bus -> TCM ECC-encode path a core store uses.
  //
  ...
  // path -- one 2:1 mux on the data-master request/response wires, off
  // the fetch critical path.
  //
  // Reset: rst_n_sync, not core_rst_n -- a warm reset restarts the
  // core from the already-verified image instead of reloading (the
  // TCMs keep their contents across a warm reset for the same reason).
  //
  // BootEnable=0 (the DEFAULT -- this design is signed off): the
  // loader is never elaborated, boot_done ties to 1, boot_active to 0,
  // and every mux below folds to a wire.  The subsystem is then
  // functionally identical to the signed-off 2026-08-24 configuration.
  ...
  if (BootEnable) begin : g_boot
    ...
    // loader absent: mux transparent, pads parked, TCM preload is the
    // bench's business (hierarchical $readmemh, as always)
