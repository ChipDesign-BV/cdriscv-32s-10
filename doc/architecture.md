...
`cdriscv_core` implements RV32IM with Zicsr and Zifencei, machine mode
only. It has two stages:

* **Fetch** (`cdriscv_if_stage`) — sequential prefetch with one
...
| Unit | File | Notes |
|------|------|-------|
| Fetch | `cdriscv_if_stage.sv` | 1 outstanding, 1 buffered instruction |
| Decode | `cdriscv_decoder.sv` | pure combinational, illegal instruction detection |
| Register file | `cdriscv_regfile.sv` | 32x32 flops, odd parity per word |
| ALU | `cdriscv_alu.sv` | one shared adder for arithmetic and compares |
| Multiply/divide | `cdriscv_multdiv.sv` | 32 iterations, data independent latency |
| Load/store | `cdriscv_lsu.sv` | single beat, misaligned access traps |
| CSR | `cdriscv_csr.sv` | M-mode subset plus two safety CSRs |
...
`cdriscv_bus` connects two masters (instruction, data) to three slaves
...
a Hsiao SEC-DED code (`cdriscv_ecc_secded.sv`, generated by
...
(`cdriscv_mbist.sv`) and a fault injection input that lets software
...
|-----------|-------|---------|
| Dual core lockstep, delayed by `LockstepDly` cycles | `cdriscv_lockstep` | faults in core logic |
| SEC-DED on both TCMs | `cdriscv_tcm` | memory bit flips |
| Odd parity on the register file | `cdriscv_regfile` | register file bit flips |
| March C- BIST | `cdriscv_mbist` | memory manufacturing and latent faults |
...
bit signals through the synchronisers in `cdriscv_sync.sv`, plus the
clock monitor's measurement result, which is captured after the toggle
that announces it.

The external reset is synchronised once (`cdriscv_rst_sync`) and drives
...
## 7. Parameters of `cdriscv_subsys`
